...
try:
    ...
    for item in old_list:
        fixed_line = checklist.remove_list_elements(item)
        f.write(fixed_line)
    f.close()
except:
    ...
    if main_menu == '4':
        ...
        while True:
            ...
            if user_delete_choice == '':
                ...
                if completed_item[1] == ' complete=False ':
                    completed_item[1] = ' complete=True '
                    completed_item[2] = str(unix_time)
                    whole_file.remove(whole_file[int(user_delete_choice) - 1])
                    whole_file.append(completed_item)
                    f = open('checklist_save_data.csv', 'w')
